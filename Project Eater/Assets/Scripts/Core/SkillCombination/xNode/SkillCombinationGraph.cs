...
/*    public SkillCombinationSlotNode[] GetSlotNodes()
    ...
    public Dictionary<(int, int), SkillCombinationSlotNode> GetSlotNodes()
        => nodes.Where(x => x != null).Cast<SkillCombinationSlotNode>().ToDictionary(node => (node.Tier, node.Index));
}
