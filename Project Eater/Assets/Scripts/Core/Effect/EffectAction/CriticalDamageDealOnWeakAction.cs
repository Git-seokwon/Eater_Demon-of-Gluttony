...
[System.Serializable]
public class CriticalDamageDealOnWeakAction : EffectAction
{
    // 기본적으로 줄 Damage
    [SerializeField]
    ...
    public override bool Apply(Effect effect, Entity user, Entity target, int level, int stack, float scale)
    {
        ...
        // 크리티컬 Apply
        if (target.SkillSystem.ContainsCategory(weakCategory))
            totalDamage = HelperUtilities.GetApplyCritDamage(totalDamage, 1f, user.Stats.CritDamageStat.Value);
        else
    ...
    protected override IReadOnlyDictionary<string, string> GetStringByKeyword(Effect effect)
    {
        var descriptionValueByKeyword = new Dictionary<string, string>
        {
            ...
            ["weakCategory"] = weakCategory.DisplayName ?? string.Empty,
        };
    ...
    {
        return new CriticalDamageDealOnWeakAction()
        {
